...
public static class ViewHelper
{
    ...
    public static void SetTextColorResource(this App.Activity activity, TextView view, int id)
    {
        view.SetTextColor((Graphics.Color)activity?.Resources?.GetColor(id));
    }
    ...
    public static void BindUp(this ScrollView scrollView, Func<int, bool> run)
    {
        ...
        scrollView.ScrollChange += (sender, e) =>
        {
            if (scrollView.MeasuredHeight > scrollView.ScrollY + scrollView.Height) return;
            if (isEnd) return;
