...
public abstract class BaseActivity : App.Activity
{
    ...
    {
        if (Build.VERSION.SdkInt < BuildVersionCodes.R || Environment.IsExternalStorageManager) return true;
        // 没有权限，需要请求权限
            ...
            switch (requestCode)
            {
                case Step2Code:
                    ...
                    for (var i = 0; i < grantResults.Length; i++)
                    {
                        ...
                        if (ret) req.Add(permissions[i]);//重新申请
                        else ban.Add(permissions[i]);//禁止
                    }
