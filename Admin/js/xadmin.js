;!function (win) {
    ...
    Xadmin.prototype.del_tab = function (id) {
        ...
        if (id) {
            ...
            parent.element.tabDelete('xbs_tab', $(window.frameElement).attr('tab-id'));
        }
    }
    ...
    Xadmin.prototype.open = function (title, url, w, h, full) {
        ...
        var index = layer.open({
            type: 2, area: [w + 'px', h + 'px'], fix: false, //不固定
            maxmin: true, shadeClose: true, shade: 0.4, title: title, content: url
        });
        if (full) {
            layer.full(index);
        }
    ...
    Xadmin.prototype.get_data = function () {
        if (typeof is_remember != "undefined") return false;
